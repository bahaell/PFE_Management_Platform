...
export default function DefenseDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params)
  const [defense, setDefense] = useState<TeacherDefense | null>(null)
  const [isLoading, setIsLoading] = useState(true)

  useEffect(() => {
    SchedulerService.getTeacherDefenseById(Number(id))
      .then(setDefense)
  ...
  if (isLoading) {
    return (
      <div className="space-y-6">
        <PageHeader title="Defense Details" description="Loading defense from scheduling-service..." />
      </div>
    )
  }

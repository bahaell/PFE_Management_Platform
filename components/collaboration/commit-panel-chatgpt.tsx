...
export function CommitPanelChatGPT({ projectId, userRole = "student" }: CommitPanelChatGPTProps) {
  ...
  const filteredCommits = useMemo(() => {
    if (selectedDocumentId === null) {
      return [...allCommits].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
    }
    return allCommits
  ...
  return (
    <div className="flex h-full bg-background flex-col lg:flex-row overflow-y">
      ...
      <div className="flex-1 flex flex-col overflow-auto min-h-0">
        <motion.div
          ...
          className="border-b border-border bg-card p-3 md:p-4 space-y-3 flex-shrink-0"
        >
          <div className="space-y-1">
            <h2 className="text-sm md:text-base font-semibold text-foreground truncate">
              {selectedDocumentId
                ? (documents && documents.find((d) => d.id === selectedDocumentId))?.name || "Document"
                : "All Feedback"}
            </h2>
            {project && <p className="text-xs text-muted-foreground line-clamp-1">{project.title}</p>}
          </div>
        ...
        <div className="flex-1 overflow-y p-3 md:p-4 space-y-4">
          <ChatLikeCommitList
        ...
        {!isReadOnly && (
          <div className="border-t border-border bg-background p-3 md:p-4 flex-shrink-0">
            <CommitInputBox
